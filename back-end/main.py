...
from db import add_inventory, delete_resident, get_all_residents, get_resident_contact, query_inventory, new_resident, query_resident, submit_inventory_collection
...
def jwt_routing(path):
    if path == 'getSignedJWT':
        succeeded, res = get_signed_jwt('ES256', request.json, request.args.get('sendToEmail') == 'true')
        if succeeded:
            return res
        else:
            return res, 400
